...
vec3 calcLight(vec3 albedo, vec3 n, vec2 lmUV, vec3 viewDir, vec3 worldPos, float smoothness, float metal, float emissive, int shadowTaps) {
	...
	// 方块光：加法叠加（"光源和材质颜色叠加"，不是乘法染色——
	// 乘法 albedo×光会把灰材质染成暗红/整体改色 = "光源替换材质"）。
	// blockAmt = max(r−g, 0)：方块光超过天光的量——白天被天光盖住
	// 自动为 0（草/沙子不染黄）；露天光斑边缘从 0 连续渐变（无阴影
	// 圈）；多光源缝隙不衰减（无"缝隙之间的阴影"）。
	// 比例（夜晚萤石旁圆石受光面）：天光乘法 ≈28% + 材质响应 ≈62%
	// + 色温蒙层 ≈10%——纹理对比与乘法版同量级（旧 0.4/0.7 的
	// 均匀光源色加法稀释纹理对比 = "圆石发灰"）、色温可见不主导。
	...
	color += albedo * blockAmt * 0.85 * dirF;                 // 材质光响应（纹理）
	color += vec3(1.0, 0.5, 0.25) * blockAmt * 0.15 * dirF;  // 光源色温蒙层
