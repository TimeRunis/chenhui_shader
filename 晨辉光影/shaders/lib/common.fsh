...
vec3 calcLight(vec3 albedo, vec3 n, vec2 lmUV, vec3 viewDir, vec3 worldPos, float smoothness, float metal, float emissive, int shadowTaps) {
	...
	// 移除 blockMask 保护（隔离测试确认"火把灰圈"根因）：保护让
	// 火把/萤石光斑内天光恢复 ×1.0（阴影处）而光斑外 ×0.5，过渡
	// 带 = smoothstep(0.003, 0.015, r-g)（1~2 级方块光，极窄）→
	// 光斑边缘天光 ×2 陡峭跳变 = "火把一圈灰色阴影"。该保护是
	// 方块光加法时代的残留（旧版加法光斑 + 天光衰减叠加更暗）；
	// 物理上屋顶遮挡天光与火把无关，光斑内天光同样被遮挡。
	// 移除后：光斑内天光 ×0.5（与光斑外一致）→ 无跳变；方块光
	// 项（乘法式物理着色）仍是光斑亮度主体，光斑观感不变
	float skyCorr = 1.0 - 0.5 * shadowAmt * df;
